...
;-
pro rad_fit_check_datetime, radar, date=date, $
...
if data_index eq -1 then begin
	...
	if ~keyword_set(noxtitle) then $
		xyouts, .5, juld- .1d, '# records', align=.5, charsize=charsize
...
; 		oplot, xrange, (*rad_fit_data[data_index]).juls[gapinds[igap]]*[1,1], col=100, linestyle=2
		xyouts, gapinds[igap], yrange[1]-1.d/24.d - gapinds[igap]*(yrange[1]-yrange[0])/(xrange[1]-xrange[0]), $
