...
def plotPanels(ind=0, start=0):
    ...
    ax1.text(9, -10, traceA.datetime[ind].strftime('%H:%M UT'), size=16, va="center", ha="center")
...
Command-line call
    ...
    force = False
    try:
        opts, args = getopt.getopt(sys.argv[1:],"hfd:",["force","date="])
        ...
        if opt == '-h':
            ...
            Options:
               -h
                   help
               -f, --force
                   force (even if image already present)
               -d<date>, --date=<date>
    ...
    try:
        date = dtmod(int(dateIn[0:4]), 
                     int(dateIn[4:6]), 
                     int(dateIn[6:]))
    except:
        date = dtmod.utcnow()

    rbspdata = '/home/sebastien/Documents/code/rbsp/data/'
    ...
    while ip < len(traceA.lat):
        filename = '{}/rbsp.orbit.{}.{:06d}.png'.format(path, strdate, ip)

        print '--> '+traceA.datetime[ip].strftime('%H:%M UT')+', '+filename
