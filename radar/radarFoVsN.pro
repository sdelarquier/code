pro radarFoVsN, date, filename, ignore_radar=ignore_radar, nops=nops, fillfov=fillfov, showplanned=showplanned, rotate=rotate, annotate=annotate, range=range, bardate=bardate, legend=legend
...
if nrad gt 0 then begin
		...
		if (radarN[0] ne '') then begin
			...
			rad_plot_fov, radarN, date=date, beam=beam, coords=coords, rotate=rotate, xpanel=[1,0], fovcolor=radarNcol, fillannotate=annotate, $
