pro radarFoVs, date, filename, ignore_radar=ignore_radar, nops=nops, fillfov=fillfov, showplanned=showplanned, rotate=rotate, annotate=annotate, range=range, bardate=bardate, legend=legend
...
if nrad gt 0 then begin
		...
		; Plot radars
		loadct, 0
		if (radarN[0] ne '') then begin
			print, '-Northern hemisphere-'
			for ir=0,n_elements(radarN)-1 do begin
					print, '!Radar found: ', radarN[ir], $
							format='(A13,2x,A3)'
			endfor
			rad_plot_fov, radarN, date=date, beam=beam, coords=coords, rotate=rotate, xpanel=[2,0], fovcolor=radarNcol, fillannotate=annotate, $
				xrange=range, yrange=range
		endif else $
			map_plot_panel, 2, 1, 0, 0, coords=coords, /iso, yrange=range, xrange=range, hemi=1, $
		...
		if (radarS[0] ne '') then begin
			...
			rad_plot_fov, radarS, date=date, beam=beam, coords=coords, rotate=rotate, xpanel=[2,1], fovcolor=radarScol, fillannotate=annotate, $
				xrange=range, yrange=range
		endif else $
			map_plot_panel, 2, 1, 1, 0, coords=coords, /iso, yrange=range, xrange=range, hemi=-1, $
					coast_linecolor=150, grid_linecolor=200, lake_fillcolor=255, rotate=rotate
