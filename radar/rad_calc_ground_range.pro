pro rad_calc_ground_range, date=date, radar=radar, beam=beam, Rgnd=Rgnd
...
fov_Re = fltarr(ngates+1)
fov_glat = fltarr(ngates+1)
fov_glon = fltarr(ngates+1)
for n=0,ngates do begin
	RadarGeoTGC, 1, fov_loc_center[0,beam,n], fov_loc_center[1,beam,n], tre, tlat, tlon, del
	fov_Re[n] = tre
	fov_glat[n] = tlat
	fov_glon[n] = tlon
endfor
