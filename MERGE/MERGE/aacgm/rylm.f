...
C
         ...
         CB = DFLOAT(L)/(L + 1.0D0)
C
         D_YLMVAL(LC) = CA * COS_THETA * D_YLMVAL(LB)
     $     - CB * D_YLMVAL(LA) 
C
50    CONTINUE
C
...
C
          ...
          LF  =   LC - 2 * M
C
          DO 180 L = M + 2, I_ORDER
C
C
            CA  = DFLOAT(2*L-1)/DFLOAT(L-M)
            CB  = DFLOAT(L+M-1)/DFLOAT(L-M)
C
            D_YLMVAL(LC) = CA * COS_THETA * D_YLMVAL(LB) - 
     $         CB * D_YLMVAL(LA)
C
            D_YLMVAL(LF) = CA * COS_THETA * D_YLMVAL(LE) - 
     $         CB * D_YLMVAL(LD)
C
