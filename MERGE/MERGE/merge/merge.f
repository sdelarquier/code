...
c
c $Log: merge.f,v $
c Revision 2.6  1996/10/01 15:05:17  senior
c - The header record now contains spare words (see info.inc,
c   where infi is now dimensionned to 32 and infr to 10.
c - Users' reponses to the option settings has been simplified:
c   It is now possible to enter 'ctrl^D' if the user chooses
c   the standard option.
c
c Revision 2.5  1996/09/19 13:09:12  senior
c - The name of the output file is now coded as YYMMDDSS, where
c   SS are the code letters for the 2 stations.
c - The output file now goes into the directory defined by the
c   environment variable SD_MRGOPEN_PATH. There is no difference
c   in the name of the file, whatever is the choice for standard
c   or non-standard options.
c - The header record of the output file contains one more word
c   to indicate if standard or non-standard options were chosen.
c   (See info.inc)
c - The program now allows for merging scan by scan. However this
c   is only possible when both radars operate with normal scans
c   (Common Time or not). For old data, if you are not sure that
c   the parameter parms386.scan was set correctly, please use
c   the option for averaging maps, and set the time integration
c   (tperiod) to whatever you wish.
c - The program now detects if the station ID changes. This is
c   possible if you have concateneted data files from different
c   radars.
c - The standard option for the integration time has changed to
c   2 minutes. The standard option for velerr_max is now 100 m/s.
c
...
c**********************************************************************
	...
	integer*2 unit_dat, unit_prt, unit_los     ! output of line of sight velocities; DA
	...
	integer*2 pnt_qlty(0:15,0:15),ngood, lgood         ! DA
	integer*4 t,st,et,time_change,CNV_MDHMS_SEC,FITROPEN,FIT_CLOSE
	...
	common/data/p,v,verr_v,verr_w,width,werr,el,el_low,el_high,
     &			bad,gsc,n_avg
	data unit_dat/3/, unit_prt/4/, unit_los/9/    ! vlos; DA
c_______________________________


C 15-MAY-1997; unit_prt opened by merge_info DA
C name changed from merge.prt to same format as .mrg and .los file DA

	rcs_rev = '$Revision: 2.6 $'
C	length = TRIM(rcs_rev)	! f90; 20000909; DA
	length = LEN(rcs_rev)
	rcs_rev(1:1) = ' '
	rcs_rev(length:length) = ' '
	print 88, rcs_rev

	call MERGE_INFO( unit_dat, unit_los, unit_prt)  ! DA
...
C Write line of sight velocities into *.los file; DA
...
c       print*,         sdate,edate,lagfr,smsep,map_qlty,lgood,niter,height_r !DA
       	if (lgood.ne.0) then
	  do ib = 0,15
	    do jb = 0,15
...
c 8000		format('========== Indices i, j:',2i13,' ==========')
c 8010		format('GG Latitude and Longitude:',2f14.4)
c 8020		format('MG Latitude and Longitude:',2f14.4)
c 8030		format('vlos12 :		  ',2f14.3)
c 8040		format('Bxyz :			  ',3f14.7)
c 8050		format('Vx Vy Vz :		  ',3f14.3)
c 8060		format('EVxyz :			  ',3f14.5)
c 8070		format('EWxyz and qlty :	  ',3f14.5,i8)
	      endif
	    enddo
	  enddo
	endif
