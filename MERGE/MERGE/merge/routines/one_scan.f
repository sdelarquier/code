	subroutine ONE_SCAN(t,fdata,fchange,tchange)
c	********************************************

c******************************************************************
c This routine reads the data files (.fit and .inx) and fills the
c radar/beam/range arrays
c For each radar, it reads the single scan that starts at time t.
c
c ATTENTION: Please use the 'Map for each scan' averaging option
c only when the two radars operate with normal scans, and when
c the parms386.scan parameter was correctly set. For all other
c cases, in particular if you are not sure that parms386.scan
c is 1 at the start of the scan and 0 otherwise, please use the
c option for averaging maps, and set the integration time to
c whatever you wish.
c
...
c
c	$Log: one_scan.f,v $
c	Revision 1.3  1996/09/19 13:29:59  senior
c	Gives the possibility of merging single scans from the 2 radars
c
c Revision 1.2  1996/09/11 10:13:18  senior
c Changed tperiod to 120 sec. for one normal scan
c
c Revision 1.1  94/09/01  16:13:54  16:13:54  senior (Catherine SENIOR)
c Initial revision
c 
c	$Revision: 1.3 $
c******************************************************************
...
c	endif
	...
	rec_time = 0
	scan = 0
	do while ((stat_read.eq.1).and.(rec_time.lt.t).and.(scan.eq.0))
...
c	    print *,' ONE_SCAN: radar, t, stat_read = ',k,t,stat_read
c	  endif
	  ...
	  do ir=1,parms386.nrang
	    ...
	    if(qflag(ir).eq.1) then
	      if(iereg.ne.1) then			!E_region elim.
	        range_gate = ir
C uses now frang and rsep; 20021003 DA
