	subroutine MERGE_INFO_DA( unit_dat, unit_prt)   ! DA
c	*********************
c
c**********************************************************************
...
c	Output only the combined los & mrg file and the prt file
c**********************************************************************
	...
	integer*4 	iostat
	integer*2 	kst
	integer*2 	unit_dat, unit_prt, unit_table     ! DA
	character*1  	ans
	character*1	NUL, TAB
...
C Modified to allow selection of al three levels of filtering in
...
c... Combined Merge + Line of sight velocities and PRT and TAB ; DA

	outf = out(:lnblnk(out)) // '.mrglos'
	outfile = MRG_NAME(outf)
	print *, ' '
	print *, 'The name of the merge output file is: ', outfile
	print *, ' '
c... changed last -> lnblnk(outfile)
	open( unit_dat,file=outfile(1:lnblnk(outfile)),status='unknown',form='unformatted')

	outf = out(:lnblnk(out)) // '.tab'
	outfile = MRG_NAME(outf)
	print *, ' '
	print *, 'The name of the vlos output file is: ', outfile
	print *, ' '
	open( unit_table,file=outfile(1:lnblnk(outfile)),status='unknown',form='unformatted')


	outf = out(:lnblnk(out)) // '.prt'
	outfile = MRG_NAME(outf)
	print *, ' '
	print *, 'The name of the prot output file is: ', outfile
	print *, ' '
	open( unit_prt,file=outfile(1:lnblnk(outfile)),status='unknown',form='formatted')
