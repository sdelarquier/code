...
$Log: beam_read.c,v $
 * Revision 1.9 20010201 DA
 * include export.h from idl and change STRING to IDL_STRING
 * Revision 1.8leic 1997/05/22  milan
     ...
     void *argv[];
{
  ...
  float pos[8],height;
  float *a1,*a2,*a3;
  long st,et,ct,*time,jndx,*icount;
  short lagfrl,smsepl,icnt,count,*lagfr;
  short *noise_lev,*noise_lag0,*noise_vel,*atten;
  short *dir,*smsep,*tfreq,*grnscat,mgflag,bmnum;
  short yr,mo,dy,hr,mn,sc,*st_id,*scan_id,err,scan;
...
/*  short int scan_id; */
  ...
  strptr1   = *(IDL_STRING *) argv[3];
  strptr2   = *(IDL_STRING *) argv[4];
  strptr3   = *(IDL_STRING *) argv[5];
  a1        = argv[6];
  a2        = argv[7];
  a3        = argv[8];
  time      = argv[9];
  ...
  if( (u1 = fopen("posg.dat","w")) == NULL )printf("ERROR: can't open posg file\n\n");
  if( (u2 = fopen("posm.dat","w")) == NULL )printf("ERROR: can't open posm file\n\n");

  printf("Date         time        scan id   lag to 1st range   gate length\n");
  printf("-----------------------------------------------------------------\n");
 ...
 * if too many occur then quit                  */
	...
	printf("ERROR: status number: %d   count: %d\n",status,count);
	if( count >= 10 ){
	    status = -1;
	    break;
	}
...
/* Status is ok - zero bad status counter */
    ...
    if ((lagfrl != dstr.p.LAGFR) || (smsepl != (int)(dstr.p.RSEP/0.15)) || (*scan_id != dstr.p.CP)) {
      printf("%02d/%02d/%04d   %02d%02d %02ds     %5d       %6.1f km       %6.1f km\n",dy,mo,yr,hr,mn,sc,dstr.p.CP,dstr.p.LAGFR*0.15,dstr.p.RSEP*1.0);
      }
...
/* Lag to first range of gate length has changed so create new pos arrays */
    if ((lagfrl != dstr.p.LAGFR) || (smsepl != (int)(dstr.p.RSEP/0.15))) {
      smsepl=(int)(dstr.p.RSEP/0.15);
      lagfrl=dstr.p.LAGFR;
      mgflag=0;
      for( ibm=0; ibm<=16; ibm++ ){
	for( igate=1; igate<=76; igate++ ){
	  rbpos_(&igate,&ibm,&lagfrl,&smsepl,&height,pos,st_id,&mgflag,&err);
	  posc[0][igate-1][ibm]=pos[0];
	  posc[1][igate-1][ibm]=pos[1];
	}
      }
      items = fwrite( posc, sizeof(posc[0][0][0]),(17*76*2),u1);
      if( items != (17*76*2) )printf("\nERROR: problem writing posg\n");
      mgflag=1;
      for( ibm=0; ibm<=16; ibm++ ){
	for( igate=1; igate<=76; igate++ ){
	  rbpos_(&igate,&ibm,&lagfrl,&smsepl,&height,pos,st_id,&mgflag,&err);
