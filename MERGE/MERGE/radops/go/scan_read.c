     ...
     void *argv[];
{
  ...
  float pos[8],height;
  float *a1,*a2,*a3;
  long st,et,ct,max_beams,*time,jndx,*icount;
  ...
  max_beams = *(long *) argv[3];
  strptr1   = *(IDL_STRING *) argv[4];
  strptr2   = *(IDL_STRING *) argv[5];
  strptr3   = *(IDL_STRING *) argv[6];
  a1        = argv[7];
  a2        = argv[8];
  a3        = argv[9];
  time      = argv[10];
  ...
  printf("Date         time        scan id   lag to 1st range   gate length   dwell\n");
  printf("-------------------------------------------------------------------------\n");
...
/* Lag to first range or gate length has changed so create new pos arrays */
