pro	rt_plot_ranelev, time, beam=beam, date=date, xrange=xrange, yrange=yrange
...
set_format, /sardines
ps_open, '~/Desktop/ranelev_'+radar+STRTRIM(beam,2)+'_'+STRTRIM(date,2)+'_'+ $
...
position = define_panel(1,2,0,0, /bar)
plot, xrange, yrange, /nodata, xstyle=5, ystyle=5, position=position, $
		...
		title=title+'!C'+subtitle, charsize=charsize
...
position = define_panel(1,2,0,1, /bar)
plot, xrange, yrange, /nodata, xstyle=5, ystyle=5, position=position
