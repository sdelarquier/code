...
;------------------------------------------------------------
pro rt_read_rays2files, radar, beam, dir=dir, nhop=nhop, back=back, outdir=outdir
...
; 	timeup = systime(/julian, /utc)
	for na=0,nazim-1 do begin
		filename = rtFileName(year*10000L+mmdd, time, radar, beam=beam[na], freq=freq, nhop=nhop, back=back, /rays, outdir=outdir)
		openw, tmpout, filename, /get_lun
		writeu, tmpout, nelev
		writeu, tmpout, reform(nrstepsave[nh,na,*])
		for nel=0,nelev-1 do begin
			writeu, tmpout, rayhour[nh], rayazim[na], rayelev[nel]
			writeu, tmpout, nel
