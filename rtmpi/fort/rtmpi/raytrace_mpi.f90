...
! MPI
	...
	WRITE(filename, '("/tmp/ranges.dat")')
	CALL MPI_FILE_OPEN(MPI_COMM_WORLD, filename, MPI_MODE_WRONLY + MPI_MODE_CREATE, MPI_INFO_NULL, hfranges, code)
	WRITE(filename, '("/tmp/ionos.dat")')
	CALL MPI_FILE_OPEN(MPI_COMM_WORLD, filename, MPI_MODE_WRONLY + MPI_MODE_CREATE, MPI_INFO_NULL, hfionos, code)
	WRITE(filename, '("/tmp/edens.dat")')
	CALL MPI_FILE_OPEN(MPI_COMM_WORLD, filename, MPI_MODE_WRONLY + MPI_MODE_CREATE, MPI_INFO_NULL, hfedens, code)
...
SUBROUTINE TRACE_RKCK(params, rayhour, rayazim, rayelev, edensARR, edensTHT, dip, hfrays, hfranges, hfionos, &
	...
	do while (ihop.lt.params%nhop.and.r.lt.(Rav + 500.)*1e3.and.theta.lt.edensTHT(500).and.nrstep.lt.5000)
		...
		! Adaptative step Runge-Kutta method (Cash-Karp)
			...
			thetatmp = theta + h*(ci(1)*thetak(1) + ci(2)*thetak(2) + ci(3)*thetak(3) + ci(4)*thetak(4) + ci(5)*thetak(5) + ci(6)*thetak(6))
