...
C 2005.02 11/10/05 updated to IGRF-11 version (S. de Larquier)
C
        subroutine igrf_sub(xlat,xlong,year,height,
     &          xl,icode,dip,dec,dipl,babs)
c-----------------------------------------------------------------------        
c INPUT:
...
c    dec       magnetic field declination
...
C ### FILMOD, DTEMOD arrays +1
        DIMENSION       GH1(144),GH2(144),GHA(144),FILMOD(15)
        DIMENSION		DTEMOD(15)
...
C ### updated to 2005
     ...
     4                  'dgrf00.dat', 'dgrf05.dat', 'igrf10.dat', 
     5                  'igrf10s.dat'/
...
667    FORMAT('/davit/lib/vt/fort/igrf/',A12)
...
C     IGRF-11 coeficients (updated by S. de Larquier, March 2011)
...
C  VALUES FOR THE NEAREST EPOCHS:
       ...
       ELSEIF (IY.LT.1930) THEN                         !1925-1930
        ...
        ELSEIF (IY.LT.2010) THEN                        !2005-2010
           ...
           F1=1.D0-F2
           G10=29554.6*F1+29496.5*F2
           G11=-1669.0*F1-1585.9*F2
           H11= 5078.0*F1+4945.1*F2
        ELSE                                            !2010-2015
