pro	iri_plot_map_diff, xmaps, ymaps, xmap, ymap, date, time=time, $
		alt=alt, scale=scale, charsize=charsize, res=res
...
; charsize=.5
title = 'IRI - 300 km - '+STRTRIM(month,2)+'/'+STRTRIM(day,2)+'/'+STRTRIM(year,2)+' - 22LT-12LT'
; clear_page
...
for nlat=1,nlats-1 do begin
	...
	rdate = STRMID(date,4,4)+','+STRMID(date,8,2)+STRMID(date,10,2)+',0,'+STRTRIM(hour[nt],2)
...
; 	print, '************************'
    ...
    nel[nlat,0:nlons/2L] = (ttnel[nlons/2L:nlons])
    nel[nlat,nlons/2L:nlons] = (ttnel[0:nlons/2L])
