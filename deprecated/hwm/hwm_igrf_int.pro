pro	hwm_igrf_int, xmaps, ymaps, xmap, ymap, date, time, alt=alt, res=res
...
; time step in sec
dt = 10.*60.
; Integration total time in hours
Tint = 1.
nint = Tint*3600./dt

; Execute hwm code
for nt=0,nint do begin
    glat = 90.
    ...
    weff[pinds] += (-(wm[pinds]*cos(dec[pinds]*!PI/180.) - wz[pinds]*sin(dec[pinds]*!PI/180.))* $
		    cos(dip[pinds]*!PI/180.)*sin(dip[pinds]*!PI/180.))*dt*1e-3
    ninds = where(dip le 0.)
    weff[ninds] += (-wm[ninds]*cos(dec[ninds]*!PI/180.) + wz[ninds]*sin(dec[ninds]*!PI/180.))* $
		    cos(dip[ninds]*!PI/180.)*sin(dip[ninds]*!PI/180.)*dt*1e-3
    
    sec += dt
endfor


; TOTAL wind efficiency plot
clear_page
position = define_panel(xmaps, ymaps, xmap, ymap, /bar, with_info=with_info)
title = 'HWM07+IGRF total '+STRMID(STRTRIM(alt,2),0,3)+'km!C'+format_date(date,/human)+' - '+STRMID(STRTRIM(hour,2),0,2)+' to '+STRMID(STRTRIM(hour+Tint,2),0,2)+'LT'
legend = 'Effective vertical diplacement [km]'
plot_map_data, weff, scale, $
    position=position, title=title, $
    legend=legend, charsize=1.5
